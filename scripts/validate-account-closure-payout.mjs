...
assert.match(closeAccount, /sendLimitedWithdrawal\(base44, walletTransaction\.id,/);
const limitedWithdrawal = await readFile(new URL('../base44/shared/limitedWithdrawal.ts', import.meta.url), 'utf8');
assert.match(limitedWithdrawal, /claimPayoutCapacity\(transactionId, cents, history\)/);
assert.match(limitedWithdrawal, /seamlessRequest\('POST', PATH_CHECK_SEND, body\)/);
