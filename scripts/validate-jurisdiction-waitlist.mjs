...
// Only a verified admin identity may process notifications.
ok(/caller\.role\s*!==\s*'admin'/.test(processSrc), 'processor denies every non-admin caller');
...
// ---------------------------------------------------------------------------
// 15. Processor auth gate: verified admin/workflow identity only.
//     Retired workflow bearer tokens no longer grant any access.
// ---------------------------------------------------------------------------
ok(/await base44\.auth\.me\(\)\.catch/.test(processSrc), 'processor verifies caller identity and fails closed');
ok(/if \(!caller\)/.test(processSrc) && /status:\s*401/.test(processSrc), 'missing and invalid sessions receive 401');
ok(/caller\.role\s*!==\s*'admin'/.test(processSrc) && /status:\s*403/.test(processSrc), 'non-admin callers receive 403');
ok(!/run_token|JURISDICTION_PROCESSOR_RUN_TOKEN|timingSafeStringEqual/.test(processSrc), 'retired bearer-token authorization is absent');
{
...
}
{
  ...
  eq(Object.keys(wfArgs).length, 0, 'workflow uses platform authentication without a reusable secret');
}
