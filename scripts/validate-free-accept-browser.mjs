...
await page.addInitScript(({scenario})=>{
 ...
 if(body.action==='readiness')return {data:{ready:false,code:'bank_required',reason:'Connect your bank.'}};
...
let s='import RefreshRuntime from "/@react-refresh";RefreshRuntime.injectIntoGlobalHook(window);window.$RefreshReg$=()=>{};window.$RefreshSig$=()=>type=>type;window.__vite_plugin_react_preamble_installed__=true;const React=(await import("/node_modules/.vite/deps/react.js")).default;const {createRoot}=(await import("/node_modules/.vite/deps/react-dom_client.js")).default;const {MemoryRouter,useLocation}=await import("/node_modules/.vite/deps/react-router-dom.js");const {HelmetProvider}=(await import("/node_modules/.vite/deps/react-helmet-async.js")).default;const Panel=(await import("/src/components/play/ChallengePanel.jsx")).default;const Market=(await import("/src/components/play/AvailableMatchSection.jsx")).default;import "/src/index.css";function App(){const location=useLocation();return location.search.includes("match=")?React.createElement("h1",null,"Ready screen reached"):React.createElement("main",{style:{maxWidth:600,margin:"auto"}},window.qa.scenario==="marketplace"?React.createElement(Market,{userId:"recipient",balance:0}):React.createElement(Panel,{inviteCode:"a".repeat(32),embedded:true}));}createRoot(document.getElementById("root")).render(React.createElement(HelmetProvider,null,React.createElement(MemoryRouter,{initialEntries:["/play?accept=1"]},React.createElement(App))));';
