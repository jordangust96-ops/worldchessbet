...
function browser({path='/',saved=null,gpc=false,token=false,broken=false}={}){
 const data={base44_app_id:'app'};if(saved)data.chessbet_cookie_preferences_v1=JSON.stringify(saved);if(token)data.base44_access_token='test-token';
...
b.p.save({analytics:true,marketing:false});ok(b.p.allowed('analytics'),'analytics accepted');ok(!b.p.allowed('marketing'),'partial choice');ok(b.calls.includes('/api/app-logs/app/log-user-in-app/home'),'Base44 initial consent page view sent');await b.ctx.fetch('/api/app-logs/app/log-user-in-app/home');ok(b.calls.filter(x=>x==='/api/app-logs/app/log-user-in-app/home').length===2,'Base44 visitor log allowed after consent');await b.ctx.fetch('/api/apps/app/analytics/track/batch');ok(b.calls.includes('/api/apps/app/analytics/track/batch'),'Base44 analytics allowed after consent');ok((await b.ctx.fetch('/api/runtime/session-recordings/ingest')).status===204,'recording remains blocked after consent');await b.ctx.fetch('https://www.google-analytics.com/g/collect');ok(b.calls.includes('https://www.google-analytics.com/g/collect'),'GA allowed');ok((await b.ctx.fetch('https://www.facebook.com/tr')).status===204,'Meta denied');b.p.save({analytics:false,marketing:false});ok(!b.p.allowed('analytics'),'withdrawn');ok((await b.ctx.fetch('/api/app-logs/app/log-user-in-app/home')).status===204,'Base44 log blocked after withdrawal');
...
b=browser({saved,token:true});ok(!b.p.allowed('analytics'),'signed-in public visit excluded');ok((await b.ctx.fetch('/api/app-logs/app/log-user-in-app/home')).status===204,'signed-in Base44 visitor log blocked');
