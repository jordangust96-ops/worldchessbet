...
const mounted = mount((name, payload) => {
  calls++;
  assert.equal(name, "getCurrentJurisdiction");
  assert.equal(payload.triggerEvent, "deposit_start");
  return new Promise(r => { resolve = r; });
});
assert.equal(calls, 0, "rendering the wallet step never checks location");
assert.equal(mounted.button.props.children, "Start deposit");
...
resolve({ data: { status: "approved", approved: true, country: "US", state: "GA", enforcementEnabled: true, vpnDetected: false } });
...
const uncertain = mount(async () => ({ data: { status: "verification_failed", approved: false, country: "US", state: "GA", enforcementEnabled: true, vpnDetected: false } }));
