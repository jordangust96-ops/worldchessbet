...
'../../shared/ledger.ts':{postLedgerLegs:async(base,args)=>{await args.beforePost();assert.ok(Math.abs(args.legs.reduce((s,l)=>s+l.debit-l.credit,0))<0.000001);assert.equal(args.updateTransactions,false);if(!batches.length){batches.push({id:'batch'});available-=args.legs[0].debit;posts++;}}}});
