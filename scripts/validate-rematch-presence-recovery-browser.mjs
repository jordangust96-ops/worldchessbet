...
const origin=process.env.REMATCH_QA_ORIGIN || 'http://localhost:5173';
 ...
 for(const id of ['p1','p2']) {
  ...
  await page.route('**/*',async route=>{
   ...
   if(u.pathname==='/__rematch-api') {
    ...
    if(body.action==='rematch_enter' && !failedEnter.has(id)){failedEnter.add(id);return route.fulfill({status:503,json:{error:'temporary registration failure'}});}
