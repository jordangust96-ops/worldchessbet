...
export default function ChallengePage() {
  ...
  const signedIn = () => {
    ...
    navigate(`/register?returnTo=${encodeURIComponent(path)}`);
    return false;
  };
  const withAction = async (name, callback) => {
    ...
    try { await callback(); }
      ...
      if (!handleChallengeGate(err,navigate,path)) {
  ...
  return <main className="min-h-screen bg-[#0A0A0A] px-5 py-7 text-white sm:py-12">
    <SEO title={card ? `${usd(card.entryAmount)} Chess Challenge | ChessBet` : 'Chess Challenge | ChessBet'} description="Open a ChessBet challenge, review the entry amount, and play someone you know. Eligibility and available funds are required." noindex />
    <Helmet><meta name="referrer" content="no-referrer" /></Helmet>
    <div className="mx-auto max-w-lg space-y-6">
      <div className="flex items-center justify-between gap-4"><Link to="/" aria-label="ChessBet"><Logo size="sm" /></Link>
      ...
      {loading ? <div className="py-16 text-center"><Loader2 className="mx-auto animate-spin text-[#C9A84C]" /></div> : card ? <>
        <section className="space-y-5 rounded-3xl border border-[#C9A84C]/20 bg-gradient-to-br from-[#191610] to-[#111] p-5 sm:p-7">
          ...
          {open && creator && <button disabled={Boolean(busy)} onClick={()=>withAction('cancel',async()=>{ await challengeRequest('cancel',{inviteCode}); await refresh(); })} className="w-full py-2 text-sm text-white/45 hover:text-red-300">Cancel Open Challenge</button>}
