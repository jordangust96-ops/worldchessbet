...
export default function CreateChallengeForm({ initialAmount = 10, rematchOf = '', onCreated, onCancel }) {
  ...
  const create = async (event) => {
    ...
    const terms = `${Number(amount)}:${rematchOf}`;
    if (request.current.terms !== terms) request.current = { terms, key:crypto.randomUUID().replaceAll('-','') };
    try {
      const result = await challengeRequest('create', { entryAmount:Number(amount), requestKey:request.current.key, ...(rematchOf ? { rematchOf } : {}) });
  ...
  return <form onSubmit={create} className="space-y-4" aria-label={rematchOf ? 'Create rematch' : 'Create shared challenge'}>
    <div><h3 className="text-lg font-bold text-white">{rematchOf ? 'Run it back' : 'Create a challenge'}</h3>
      <p className="mt-1 text-sm text-white/50">5-minute Blitz · No increment</p></div>
    ...
    <p className="text-xs leading-relaxed text-white/50">Creating a link reserves no money. Both players must have available funds and pass eligibility checks when the challenge is accepted. The winner award includes both entry amounts; the fee is separate.</p>
