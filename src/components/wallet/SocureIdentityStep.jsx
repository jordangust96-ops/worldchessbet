...
export default function SocureIdentityStep({ identity, onRefresh, locationApproved = false }) {
  ...
  const start = async () => {
    ...
    try {
      const { data } = await base44.functions.invoke("startSocureIdentityVerification", { consent: true });
  ...
  return <WalletSetupStep number={2} label="Identity verification" title={titles[status] || "Identity status unavailable"} complete={status === "verified"} pending={["pending", "review_required"].includes(status)} attention={["rejected", "failed", "expired", "incomplete"].includes(status)}
    ...
    {canStart && <>
      <button type="button" onClick={start} disabled={busy} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl gold-gradient px-4 py-3 text-sm font-semibold text-black disabled:opacity-40 sm:w-auto">
