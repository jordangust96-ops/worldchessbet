...
// Generic admin editor for any legal document type, mirroring the original
// Privacy Policy admin flow but scoped by policyType so each document keeps
// its own independent version history.
export default function LegalDocumentAdmin({ policyType }) {
  ...
  const load = async () => {
    ...
    setIsAdmin(true);
    const configs = await base44.entities.PrivacyPolicyConfig.filter({ policy_type: policyType }, "-version");
    setHistory(configs);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] px-5 pt-8 pb-16">
      ...
      <h1 className="text-xl font-extrabold text-white mb-1">Manage {docMeta.label}</h1>
      ...
      <div className="space-y-4 max-w-2xl">
        ...
        <div>
          <label className="text-xs font-semibold text-white/50 uppercase tracking-wider">
            {docMeta.label} Content (Markdown, use ## for section headings)
          </label>
          <textarea
