...
import { verifyMatchLocation, getMatchLocationReadiness, verifyChallengeCreationLocation } from './matchLocation.ts';
...
export async function createChallenge(base44: any, user: any, body: any, req: Request, rematchContext: any = null) {
  if (body.rematchOf && !rematchContext) fail('result_screen_required','Request a rematch from the result screen.',409);
  ...
  if (!free && !(rematchContext ? validEntry(body.entryAmount) : validNewEntry(body.entryAmount))) fail('invalid_entry', 'Choose one of the available Entry Amounts.', 400);
  ...
  const serviceFee=rematchContext ? rematchContext.serviceFee : (free ? 0 : getPlatformServiceFee(Number(body.entryAmount)));
  if (!Number.isFinite(serviceFee) || serviceFee<0 || Math.abs(serviceFee*100-Math.round(serviceFee*100))>0.000001 || Number(body.serviceFee) !== serviceFee)
  ...
  return withChallengeCreationLock(user.id, async checkLease => {
    ...
    const terms = { play_mode:free?'free':'money', challenge_version:CHALLENGE_VERSION, is_private:true, wager_amount:Number(body.entryAmount), platform_service_fee:serviceFee };
    ...
    await checkLease();
    if(rematchContext) {
      await rematchContext.check();
      await requireRematchPresence(base44,{challenge_in_screen_rematch:true,challenge_rematch_of:rematchOf,player1_id:user.id,challenge_target_id:targetId,challenge_rematch_presence:rematchContext.presence});
    }
    ...
    let match = await base44.asServiceRole.entities.Match.create({
      ...
      platform_service_fee: serviceFee, platform_fee_schedule_version: rematchContext?.feeScheduleVersion || PLATFORM_FEE_SCHEDULE_VERSION,
      time_control: timeControl.value, display_name: rematchContext?.displayName || timeControl.displayName, clock_initial_ms: Number(rematchContext?.clockMs)>0 ? Number(rematchContext.clockMs) : timeControl.clockMs,
      ...
      ...(rematchContext ? {challenge_in_screen_rematch:true,challenge_rematch_presence:rematchContext.presence} : {}),
      challenge_expires_at: rematchContext?.expiresAt || new Date(Date.now() + CHALLENGE_TTL_MS).toISOString(),
      ...
      player1_certified:false, player2_certified:false, notify_on_accept:!rematchContext,
    ...
    if (!free) {
      match = await underMatchLock(base44,match.id,(fresh,owner)=>underWalletLocks([user.id],owner,fresh.id,async()=>{
        return finishCreatorReservation(base44,fresh,owner,async()=>{
          ...
          if(rematchContext) {await rematchContext.check();await requireRematchPresence(base44,latest);}
          return true;
        });
      }));
    }
...
export async function acceptChallenge(req: Request, base44: any, user: any, match: any, body: any, rematchContext: any = null) {
  if(match.challenge_in_screen_rematch && !rematchContext) fail('result_screen_required','Accept this rematch on the result screen.',409);
  ...
  return withChallengePlayersLock([match.player1_id,user.id], checkPlayerLeases => underMatchLock(base44, match.id, async (fresh, owner) => {
    if (isFreeMatch(fresh)) assertFreeMatch(fresh);
    ...
    if(rematchContext) {await rematchContext.check();await requireRematchPresence(base44,fresh);}
...
async function closeFreeChallenge(base44: any, match: any, reason: string) {
  assertFreeMatch(match);
  const games=await base44.asServiceRole.entities.Game.filter({match_id:match.id},'created_date',1);
  if (games[0]) fail('already_started','This game has already started.');
  const updated=await base44.asServiceRole.entities.Match.update(match.id,{
    status:'cancelled',result:'cancelled',challenge_close_reason:reason,challenge_authorized_until:nowIso(),
  });
  return {match:updated};
}

export async function cancelChallenge(base44: any, user: any, matchId: string, reason = 'cancelled') {
  const attempt = () => underMatchLock(base44, matchId, async (match, owner) => {
    ...
    if (isFreeMatch(match)) return closeFreeChallenge(base44,match,reason);
    ...
    if (match.status === 'searching' && !activeOperation(match) && !match.player2_id && !match.challenge_claimant_id) {
      ...
      if (!committed && !await batchFor(base44,creatorReservationGroup(match))) {
        ...
        await challengeEvent(base44, updated, 'closed', user.id, reason);
        return { match: updated };
      }
    }
    ...
    const ids = [match.player1_id, match.player2_id || match.challenge_claimant_id].filter(Boolean);
    return underWalletLocks(ids, owner, match.id, () => releaseChallengeLocked(base44, match, owner, reason));
  });
...
export async function finalizeChallengeStart(base44: any, user: any, matchId: string) {
  return underMatchLock(base44, matchId, async (match, owner) => {
    ...
    if (challengeStartExpired(match)) fail('ready_expired', isFreeMatch(match) ? 'The start window expired. This free game will close.' : 'The start window expired. This match will close and release both reservations.');
    if (!bothChallengePlayersReady(match)) return { match, waitingForReady: true };
    if (!match.player1_certified || !match.player2_certified || (!isFreeMatch(match) && (!match.player1_deposited || !match.player2_deposited)))
...
export async function recoverChallenge(base44: any, matchId: string) {
  return underMatchLock(base44, matchId, async (match, owner) => {
    ...
    if (isFreeMatch(match)) {
      assertFreeMatch(match);
      if (match.status==='cancelled') return {skipped:true};
      if ((match.status==='searching' && challengeExpired(match)) ||
          (['preparing','both_ready'].includes(match.status) && challengeStartExpired(match))) {
        const games=await base44.asServiceRole.entities.Game.filter({match_id:match.id},'created_date',1);
        if(games[0] && match.start_operation_id) {
          const updated=await base44.asServiceRole.entities.Match.update(match.id,{status:'in_progress',game_id:games[0].id});
          return {match:updated,recovered:true};
        }
        return closeFreeChallenge(base44,match,'timeout');
      }
      return {skipped:true};
    }
