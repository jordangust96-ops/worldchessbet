import { getMatchLocationReadiness, matchLocationRequiredResponse } from './matchLocation.ts';

import { challengeClockMs, isFreeMatch, assertFreeMatch } from './challengePolicy.js';
...
export async function getOrCreateMatchGame(base44: any, user: any, body: any) {
  try {
    if (!user) return Response.json({ error: 'Unauthorized' }, { status: 401 });

    const { matchId, challengeStartOwner } = body;
    ...
    if (Number(match.challenge_version) === 1 && !match.game_id) {
      ...
      const { refreshContestLocks } = await import('./seamlessAtomicStore.ts');
      const { bothChallengePlayersReady, challengeStartExpired } = await import('./challengePolicy.js');
