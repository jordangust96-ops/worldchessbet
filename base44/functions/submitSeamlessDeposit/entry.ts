...
const MAX_AMOUNT = 1093.4; // $1,100 bank debit limit less self-funding deposit fee.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!quote) return Response.json({ error: 'Enter a deposit between $10.00 and $1,093.40 (bank debit limit includes the fee).' }, { status: 400 });
